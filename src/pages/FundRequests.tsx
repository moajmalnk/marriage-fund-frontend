...
const FundRequests = () => {
  ...
  // Fetch Dashboard Stats for Limit Calculation
  ...
  // Logic: (Total Non-Admin Members - 1) * 5000
  const totalMembers = (dashboardStats?.demographics?.married || 0) + (dashboardStats?.demographics?.unmarried || 0);
  
  // If stats loaded, calculate real limit. Fallback to 120,000 only if loading.
  const maxAmount = totalMembers > 0 
    ? Math.max(0, (totalMembers - 1) * 5000) 
    : (dashboardStats ? 0 : 120000);

  const isAmountOverLimit = Number(formData.amount) > maxAmount;
  ...
  const handleCreateSubmit = () => {
    ...
    if (isAmountOverLimit) return; // Security Check
  ...
  const getStatusBadge = (request: any) => {
    ...
    if (status === 'approved') {
      if (paymentStatus === 'paid') {
        return <Badge className="bg-green-100 text-green-800 border-green-200 dark:bg-green-950 dark:text-green-300 dark:border-green-800"><CheckCircle className="mr-1 h-3 w-3" /> Paid (₹{Number(request.paid_amount || request.amount).toLocaleString('en-IN')})</Badge>;
      } else if (paymentStatus === 'partial') {
        return <Badge className="bg-yellow-100 text-yellow-800 border-yellow-200 dark:bg-yellow-950 dark:text-yellow-300 dark:border-yellow-800"><CreditCard className="mr-1 h-3 w-3" /> Partial</Badge>;
      } else {
        const remainingDays = request.scheduled_payment_date ? getRemainingDays(request.scheduled_payment_date) : 0;
        return <Badge className="bg-blue-100 text-blue-800 border-blue-200 dark:bg-blue-950 dark:text-blue-300 dark:border-blue-800"><Timer className="mr-1 h-3 w-3" /> {remainingDays > 0 ? `${remainingDays} days left` : 'Due'}</Badge>;
      }
  ...
  const approvedAmount = requests.filter((r: any) => normalizeStatus(r.status) === 'approved').reduce((sum: number, r: any) => sum + Number(r.amount), 0);
  const pendingAmount = requests.filter((r: any) => normalizeStatus(r.status) === 'pending').reduce((sum: number, r: any) => sum + Number(r.amount), 0);
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-fade-in">
      ...
      {/* Statistics Cards - ... (Kept same as before) ... */}
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3">
        <Card className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/30 border-blue-200 dark:border-blue-800">
          <CardContent className="p-3 sm:p-4 lg:p-6">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="p-1.5 sm:p-2 rounded-lg sm:rounded-xl bg-blue-500 text-white flex-shrink-0"><FileText className="h-4 w-4 sm:h-5 sm:w-5" /></div>
        ...
        <Card className="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/30 dark:to-emerald-950/30 border-green-200 dark:border-green-800">
          <CardContent className="p-3 sm:p-4 lg:p-6">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="p-1.5 sm:p-2 rounded-lg sm:rounded-xl bg-green-500 text-white flex-shrink-0"><CheckCircle className="h-4 w-4 sm:h-5 sm:w-5" /></div>
              <div className="min-w-0 flex-1">
        ...
        <Card className="bg-gradient-to-br from-orange-50 to-red-50 dark:from-orange-950/30 dark:to-orange-950/30 border-orange-200 dark:border-orange-800">
          <CardContent className="p-3 sm:p-4 lg:p-6">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="p-1.5 sm:p-2 rounded-lg sm:rounded-xl bg-orange-500 text-white flex-shrink-0"><Clock className="h-4 w-4 sm:h-5 sm:w-5" /></div>
              <div className="min-w-0 flex-1">
        ...
        <Card className="bg-gradient-to-br from-red-50 to-pink-50 dark:from-red-950/30 dark:to-pink-950/30 border-red-200 dark:border-red-800">
          <CardContent className="p-3 sm:p-4 lg:p-6">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="p-1.5 sm:p-2 rounded-lg sm:rounded-xl bg-red-500 text-white flex-shrink-0"><XCircle className="h-4 w-4 sm:h-5 sm:w-5" /></div>
              <div className="min-w-0 flex-1">
        ...
        <Card className="bg-gradient-to-br from-purple-50 to-violet-50 dark:from-purple-950/30 dark:to-violet-950/30 border-purple-200 dark:border-purple-800">
          <CardContent className="p-3 sm:p-4 lg:p-6">
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="p-1.5 sm:p-2 rounded-lg sm:rounded-xl bg-purple-500 text-white flex-shrink-0"><DollarSign className="h-4 w-4 sm:h-5 sm:w-5" /></div>
              <div className="min-w-0 flex-1">
      ...
      {canManageRequests && (
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'all' | 'my')} className="space-y-4">
          <TabsList className="grid w-full grid-cols-2 h-12 bg-slate-100 dark:bg-slate-800">
            <TabsTrigger value="all" className="text-sm font-medium data-[state=active]:bg-white data-[state=active]:text-slate-900 data-[state=active]:shadow-sm dark:data-[state=active]:bg-slate-700 dark:data-[state=active]:text-slate-100">All Requests</TabsTrigger>
            <TabsTrigger value="my" className="text-sm font-medium data-[state=active]:bg-white data-[state=active]:text-slate-900 data-[state=active]:shadow-sm dark:data-[state=active]:bg-slate-700 dark:data-[state=active]:text-slate-100">My Requests</TabsTrigger>
          </TabsList>
        </Tabs>
      )}

      {/* Requests Table - ... (Kept same as before) ... */}
      <Card className="bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-700 border-slate-200 dark:border-slate-600">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-slate-900 dark:text-slate-100">
            <div className="p-2 rounded-lg bg-slate-500 text-white"><MessageSquare className="h-5 w-5" /></div>
            {activeTab === 'all' ? 'All Fund Requests' : 'My Fund Requests'}
          </CardTitle>
          <CardDescription className="text-slate-700 dark:text-slate-300">Review and manage fund assistance requests</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-100 dark:bg-slate-800">
                  ...
                  {isAdmin && <TableHead className="font-semibold text-slate-900 dark:text-slate-100 w-12 text-right">Actions</TableHead>}
                </TableRow>
              </TableHeader>
              <TableBody>
                {filteredRequests.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center py-12">
                      <div className="flex flex-col items-center gap-3">
                        <div className="p-3 rounded-full bg-slate-100 dark:bg-slate-800"><FileText className="h-6 w-6 text-slate-400" /></div>
                  ...
                  filteredRequests.map((request: any) => (
                    <TableRow key={request.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      <TableCell className="font-medium text-slate-900 dark:text-slate-100">
                        <div className="flex items-center gap-3">
                          <Avatar className="h-8 w-8">
                            ...
                            <AvatarFallback className="bg-primary/10 flex items-center justify-center"><Users className="h-4 w-4 text-primary" /></AvatarFallback>
                          </Avatar>
                          <div>
                      ...
                      <TableCell className="max-w-xs hidden sm:table-cell"><p className="truncate text-slate-700 dark:text-slate-300" title={request.reason}>{request.reason}</p></TableCell>
                      <TableCell className="text-slate-600 dark:text-slate-400">{new Date(request.requested_date).toLocaleDateString('en-IN')}</TableCell>
                      <TableCell>{getStatusBadge(request)}</TableCell>
                      <TableCell className="hidden md:table-cell">
                        {normalizeStatus(request.status) === 'approved' && request.scheduled_payment_date ? (
                          <div className="space-y-1"><p className="text-xs text-slate-600 dark:text-slate-400">Due: {new Date(request.scheduled_payment_date).toLocaleDateString()}</p></div>
                        ...
                        <TableCell className="text-right">
                          {normalizeStatus(request.status) === 'pending' ? (
                            <div className="flex justify-end gap-1">
                              <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white text-xs px-2 py-1" onClick={() => handleApproveRequest(request)}>
                                <CheckCircle className="h-3 w-3 sm:mr-1" /><span className="hidden sm:inline">Approve</span>
                              </Button>
                              <Button size="sm" variant="destructive" className="bg-red-600 hover:bg-red-700 text-xs px-2 py-1" onClick={() => handleDeclineRequest(request)}>
                                <XCircle className="h-3 w-3 sm:mr-1" /><span className="hidden sm:inline">Decline</span>
                              </Button>
                            </div>
                          ) : (
                            <span className="text-xs text-slate-500 dark:text-slate-400 capitalize">{request.status.toLowerCase()}</span>
                          )}
                        </TableCell>
                      )}
                    </TableRow>
      ...
      {/* Create Request Dialog - UPDATED WITH VALIDATION */}
      <Dialog open={showCreateModal} onOpenChange={setShowCreateModal}>
        <DialogContent className="sm:max-w-[500px] max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <div className="p-2 rounded-lg bg-blue-500 text-white"><Heart className="h-5 w-5" /></div>
              Request Fund
            </DialogTitle>
            <DialogDescription>Submit a request for marriage fund assistance from the community.</DialogDescription>
          </DialogHeader>
            ...
            <div className="grid gap-2">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  className={`h-12 pl-10 focus:border-blue-500 ${isAmountOverLimit ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'border-slate-300'}`}
                  min="1"
                  max={maxAmount}
                />
              </div>
              ...
              {isAmountOverLimit && (
                <Alert variant="destructive" className="mt-2 bg-red-50 dark:bg-red-950/30 border-red-200 dark:border-red-800">
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription className="text-xs">
                    <span className="font-semibold block mb-1">Limit Exceeded</span>
                    The maximum requested amount cannot exceed <strong>₹{maxAmount.toLocaleString('en-IN')}</strong>. This limit is based on the current number of contributing members ({(totalMembers - 1)} members × ₹5,000).
                  </AlertDescription>
                </Alert>
              )}
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={createMutation.isPending || !formData.amount || !formData.reason || isAmountOverLimit} 
            ...
            >
               {createMutation.isPending  ? (
                ...
                <><Heart className="h-4 w-4 mr-2" /> Submit Request</>
              )}
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={showApproveModal} onOpenChange={closeModals}>
       <DialogContent className="sm:max-w-[500px] max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><div className="p-2 rounded-lg bg-green-500 text-white"><CheckCircle className="h-5 w-5" /></div> Approve Fund Request</DialogTitle>
            <DialogDescription>Schedule payment for {selectedRequest?.user_name}</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
              <h4 className="font-medium text-slate-900 dark:text-slate-100 mb-3">Request Details</h4>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-slate-600 dark:text-slate-400">Member:</span><span className="font-medium text-slate-900 dark:text-slate-100">{selectedRequest?.user_name}</span></div>
                <div className="flex justify-between"><span className="text-slate-600 dark:text-slate-400">Amount:</span><span className="font-medium text-green-600 dark:text-green-400">₹{Number(selectedRequest?.amount).toLocaleString('en-IN')}</span></div>
                <div className="flex justify-between"><span className="text-slate-600 dark:text-slate-400">Reason:</span><span className="font-medium text-slate-900 dark:text-slate-100">{selectedRequest?.reason}</span></div>
              </div>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="payment-date" className="text-sm font-medium">Payment Date</Label>
              <DatePicker value={paymentDate ? paymentDate.toISOString().split('T')[0] : ''} onChange={(dateString) => setPaymentDate(new Date(dateString))} placeholder="Select payment date" />
              <p className="text-xs text-slate-500 dark:text-slate-400">Payment will be scheduled for the selected date (typically 45 days from approval)</p>
            </div>
              <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800">
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="text-blue-700 dark:text-blue-300">This will approve the fund request and schedule payment. The member will be notified of the approval and payment date.</p>
                  </div>
                </div>
              </div>
          </div>
          <DialogFooter>
            ...
            <Button onClick={processApproval} disabled={approveMutation.isPending || !paymentDate} className="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white">
              {approveMutation.isPending ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin mr-2" /> Processing...</> : <><CheckCircle className="h-4 w-4 mr-2" /> Approve Request</>}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showDeclineModal} onOpenChange={closeModals}>
       <DialogContent className="sm:max-w-[500px] max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><div className="p-2 rounded-lg bg-red-500 text-white"><XCircle className="h-5 w-5" /></div> Decline Fund Request</DialogTitle>
            <DialogDescription>Provide reason for declining {selectedRequest?.user_name}'s request</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-4">
              <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between"><span className="text-slate-600 dark:text-slate-400">Member:</span><span className="font-medium text-slate-900 dark:text-slate-100">{selectedRequest?.user_name}</span></div>
                  <div className="flex justify-between"><span className="text-slate-600 dark:text-slate-400">Amount:</span><span className="font-medium text-red-600 dark:text-red-400">₹{selectedRequest?.amount.toLocaleString('en-IN')}</span></div>
                  <div className="flex justify-between"><span className="text-slate-600 dark:text-slate-400">Reason:</span><span className="font-medium text-slate-900 dark:text-slate-100">{selectedRequest?.reason}</span></div>
                </div>
              </div>
              <div className="grid gap-2">
                <Label htmlFor="decline-reason" className="text-sm font-medium">Reason for Declining *</Label>
                <Textarea id="decline-reason" placeholder="Please provide a clear reason for declining this fund request..." value={declineReason} onChange={(e) => setDeclineReason(e.target.value)} className="min-h-[100px] border-slate-300 focus:border-red-500" />
                <p className="text-xs text-slate-500 dark:text-slate-400">This reason will be shared with the member</p>
              </div>
              <div className="p-4 rounded-xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800">
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="text-red-700 dark:text-red-300">This will decline the fund request. The member will be notified with the reason provided.</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={processDecline} disabled={declineMutation.isPending || !declineReason.trim()} className="bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white">
               {declineMutation.isPending ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin mr-2" /> Processing...</> : <><XCircle className="h-4 w-4 mr-2" /> Decline Request</>}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showTermsDialog} onOpenChange={setShowTermsDialog}>
        <DialogContent className="w-[95vw] max-w-[95vw] sm:w-auto sm:max-w-2xl mx-2 sm:mx-4 h-[90vh] sm:h-auto max-h-[90vh] sm:max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><div className="p-2 rounded-lg bg-indigo-500 text-white"><FileCheck className="h-5 w-5" /></div> Terms of Use Required</DialogTitle>
            <DialogDescription>You must acknowledge the Terms of Use before requesting funds</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-lg p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="font-semibold text-amber-900 dark:text-amber-100 mb-2">Terms of Use Required</h4>
                  <p className="text-amber-800 dark:text-amber-200 text-sm">Before you can request funds from the CBMS Marriage Fund, you must read and acknowledge our Terms of Use. This ensures you understand the rules, responsibilities, and procedures of the fund.</p>
                </div>
              </div>
            </div>
            <div className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-4">
              <h4 className="font-semibold text-slate-900 dark:text-slate-100 mb-2">Key Points from Terms of Use:</h4>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleTermsAcknowledgment} className="bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white">
              <FileCheck className="h-4 w-4 mr-2" /> I Acknowledge & Accept Terms
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showWalletModal} onOpenChange={setShowWalletModal}>
        <DialogContent className="sm:max-w-[500px] max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><div className="p-2 rounded-lg bg-green-500 text-white"><Wallet className="h-5 w-5" /></div> Deposit to Wallet</DialogTitle>
            <DialogDescription>Add funds to your personal wallet for future use.</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-4">
              <div className="grid gap-2">
                <Label htmlFor="wallet-amount" className="text-sm font-medium">Deposit Amount (₹)</Label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-500 dark:text-slate-400">₹</span>
                  <Input id="wallet-amount" type="number" placeholder="Enter amount" value={walletFormData.amount} onChange={(e) => handleWalletInputChange('amount', e.target.value)} className="h-12 pl-10 border-slate-300 focus:border-green-500" required min="1" disabled={isSubmittingWallet} />
                </div>
              </div>
              <div className="grid gap-2">
                <Label htmlFor="wallet-payment-method" className="text-sm font-medium">Payment Method</Label>
                <Select value={walletFormData.paymentMethod} onValueChange={(value) => handleWalletInputChange('paymentMethod', value)} disabled={isSubmittingWallet}>
                  <SelectTrigger className="h-12 border-slate-300 focus:border-green-500"><SelectValue placeholder="Select payment method" /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="grid gap-2">
                <Label htmlFor="wallet-transaction-id" className="text-sm font-medium">Transaction ID / Reference Number</Label>
                <Input id="wallet-transaction-id" type="text" placeholder="Enter transaction ID or reference number" value={walletFormData.transactionId} onChange={(e) => handleWalletInputChange('transactionId', e.target.value)} className="h-12 border-slate-300 focus:border-green-500" required disabled={isSubmittingWallet} />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="wallet-notes" className="text-sm font-medium">Additional Notes (Optional)</Label>
                <Textarea id="wallet-notes" placeholder="Any additional information about this deposit..." value={walletFormData.notes} onChange={(e) => handleWalletInputChange('notes', e.target.value)} className="min-h-[100px] border-slate-300 focus:border-green-500" disabled={isSubmittingWallet} />
              </div>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowWalletModal(false)} disabled={isSubmittingWallet}>Cancel</Button>
            <Button onClick={handleWalletSubmit} disabled={isSubmittingWallet || !walletFormData.amount || !walletFormData.transactionId} className="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white">
              {isSubmittingWallet ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin mr-2" /> Submitting...</> : <><Wallet className="h-4 w-4 mr-2" /> Submit Deposit</>}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
